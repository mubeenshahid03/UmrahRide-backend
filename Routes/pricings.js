...
router.post("/addpricing", async (request, response) => {
  try {
    const already = await Pricing.findOne({
    ...
    if (already) {
      return response
        ...
        .json({
          error:
            "this destination id and vehicleid is already in pricing table",
        });
    } else {
...
router.get("/:destinationId", async (request, response) => {
  try {
    ...
    if (!pricingInfo) {
      return response
        .status(404)
        .json({ error: "Pricing information not found for the destination" });
    }
